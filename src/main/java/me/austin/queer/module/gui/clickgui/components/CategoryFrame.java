...
public class CategoryFrame extends Component {
	...
	public CategoryFrame(Category category, int x, int y, ClickGuiScreen parent) {
		super(category.getName(), category.getDescription(), x, y, 80 * wpixel, parent.hacks.getHacksByCategory(category).size() * 20);
	...
	public void render(MatrixStack matrices, TextRenderer textRenderer, int x, int y) {
		...
		Screen.fill(null, x, y, x + width, y + height, new Color(red, green, blue, 50).getRGB());
	...
	public void mouseClicked(double mousex, double mousey, int key) {
		if (clickCheck(mousex, mousey, x, y, width, height)) {
			this.dragging = true;
		}

		for (HackButton button : buttons) {
			if (clickCheck(mousex, mousey, button.x, button.y, button.width, button.height)) {
