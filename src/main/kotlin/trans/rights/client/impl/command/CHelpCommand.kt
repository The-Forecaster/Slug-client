...
object CHelpCommand : Command("chelp", "Informs you about the different features of this client", "/chelp") {
    private fun helpMessage(context: CommandContext<*>): Int {
        ChatHelper.send(TransRights.NAME)

        CommandManager.values.forEach { command ->
            ChatHelper.send("${command.name} : ${command.description} : ${command.syntax}", false)
        }
    ...
    override fun register(dispatcher: CommandDispatcher<ServerCommandSource>) {
        dispatcher.register(literal("chelp").executes(this::helpMessage))
    }
}
