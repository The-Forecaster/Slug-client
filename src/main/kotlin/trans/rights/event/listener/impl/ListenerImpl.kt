...
inline fun <reified T : Any> Any.lambdaListener(noinline action: (T) -> Unit, priority: Int) : LambdaListener<T> {
    return listener(action, priority, this, T::class.java)
}

fun <T: Any> listener(action: (T) -> Unit, priority: Int, parent: Any, target: Class<T>): LambdaListener<T> {
    return LambdaListener(action, priority, parent, target)
}

/**
 * Implementation of Listener that uses a consumer function as its target
 */
