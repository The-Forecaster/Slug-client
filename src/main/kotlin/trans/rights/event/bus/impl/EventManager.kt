...
class EventManager(
        ...
        override val subscribers:
                ConcurrentHashMap<Class<*>, CopyOnWriteArraySet<MethodListener<*>>> =
                ConcurrentHashMap()
) : EventBus {
    override fun register(subscriber: Any) {
        if (this.registeredListeners.contains(subscriber)) return

        Arrays.stream(subscriber.javaClass.methods)
                .filter { method ->
                ...
                .forEach { method ->
                    subscribers
                            .getOrPut(method.parameters[0].javaClass, ::CopyOnWriteArraySet)
                            .add(
                                    MethodListener(
                                            method,
